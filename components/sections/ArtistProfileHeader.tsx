              ...
              <div className="flex items-center gap-3">
                {(artist.contactNumber && artist.contactNumber.trim() !== "") && (
                  <button
                    ...
                    className="w-12 h-12 rounded-full bg-[#007AFF] text-white flex items-center justify-center shadow-md"
                  >
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.44-5.15-3.75-6.59-6.59l1.97-1.57c.3-.29.4-.71.23-1.1C9 6.65 8.79 5.43 8.79 4.18 8.79 3.53 8.26 3 7.61 3H4.19C3.54 3 3 3.53 3 4.18c0 9.8 7.99 17.79 17.79 17.79.65 0 1.18-.54 1.18-1.19v-3.41c0-.65-.54-1.19-1.19-1.19z" />
                    </svg>
                  </button>
                )}
                ...
                {(artist.whatsappNumber && artist.whatsappNumber.trim() !== "") && (
                  <button
                    onClick={onWhatsApp}
                    aria-label="WhatsApp artist"
                    className="w-12 h-12 rounded-full bg-[#25D366] text-white flex items-center justify-center shadow-md"
                  >
        ...
        <div className="relative min-h-[600px] rounded-2xl overflow-hidden mx-4">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
            ...
            <div className="flex gap-4">
              <button
                onClick={handleRequestBooking}
                className="flex-1 bg-gradient-to-r from-primary-orange to-primary-pink text-white py-3 px-4 rounded-full font-medium hover:shadow-lg hover:shadow-primary-pink/25 transition-all duration-300"
              >
                Request Booking
              </button>
              ...
              {(artist.contactNumber && artist.contactNumber.trim() !== "") && (
                <button
                  onClick={onCall}
                  className="w-12 h-12 bg-[#007AFF] text-white rounded-full font-medium hover:bg-blue-600 transition-colors flex items-center justify-center shadow-lg shadow-blue-500/20"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.44-5.15-3.75-6.59-6.59l1.97-1.57c.3-.29.4-.71.23-1.1C9 6.65 8.79 5.43 8.79 4.18 8.79 3.53 8.26 3 7.61 3H4.19C3.54 3 3 3.53 3 4.18c0 9.8 7.99 17.79 17.79 17.79.65 0 1.18-.54 1.18-1.19v-3.41c0-.65-.54-1.19-1.19-1.19z" />
                  </svg>
              ...
              {(artist.whatsappNumber && artist.whatsappNumber.trim() !== "") && (
                <button
                  onClick={onWhatsApp}
                  className="w-12 h-12 bg-[#25D366] text-white rounded-full font-medium hover:bg-[#20bd5a] transition-colors flex items-center justify-center shadow-lg shadow-green-500/20"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488" />
                  </svg>
