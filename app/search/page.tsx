...
export default function MobileSearchPage() {
  ...
  useEffect(() => {
    if (!search.trim()) {
      setFilteredArtists([]);
      setCategories(ARTIST_CATEGORIES);
      return;
    }
    setFilteredArtists(
      artists.filter(
  ...
  const handleCategoryClick = (catValue: string) => {
    router.push(`/artists?type=${encodeURIComponent(catValue)}`);
  };

  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      {/* Top Bar */}
      <div className="p-4 pb-8 py-4 flex flex-col">
